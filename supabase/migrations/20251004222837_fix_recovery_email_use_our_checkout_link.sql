/*
  # Corrigir Email de Recuperação - Usar Link do Nosso Checkout

  1. Objetivo
    - Modificar a função de recuperação para usar o link do NOSSO checkout
    - Em vez de usar secure_url (link da Bestfy), usar /checkout/{slug}

  2. Mudanças
    - Buscar checkout_slug da tabela checkout_links usando payment_id
    - Construir URL: /checkout/{checkout_slug}
    - Só enviar email se o checkout_links existir
...
-- Create improved function to send recovery emails using OUR checkout links
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      checkout_url := '/checkout/' || payment_record.checkout_slug;
      ...
      RAISE NOTICE '[AUTO-RECOVERY] ✅ Email queued for % (request_id: %) - Checkout: %', 
        payment_record.customer_email, 
        request_id,
        checkout_url;
...
DO $$
BEGIN
  RAISE NOTICE '✅ Recovery email system updated to use OUR checkout links!';
END $$;