/*
  # Fix Recovery Emails to Use Database Token

  1. Updates the automatic recovery email function to:
     - Read Postmark token from api_keys table
     - Skip if no token is configured
     - Log properly when token is missing
  
  2. Security:
     - Function still uses SECURITY DEFINER
     - No changes to RLS policies
*/

-- Drop and recreate the function with proper token handling
...
BEGIN
  ...
  SELECT api_key INTO postmark_token
  FROM api_keys
  ...
  RAISE NOTICE '[AUTO-RECOVERY] ✅ Postmark token found, proceeding with email sending';
